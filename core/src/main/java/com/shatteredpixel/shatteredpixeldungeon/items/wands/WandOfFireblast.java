...
public class WandOfFireblast extends DamageWand {
	...
	public List<Integer> aimTiles(int target) {
		...
		ConeAOE tempCone = new ConeAOE( b,
				...
				Ballistica.STOP_TARGET | Ballistica.STOP_SOLID | Ballistica.IGNORE_SOFT_SOLID | Ballistica.FOGOFWAR);
	...
	public String statsDesc() {
		if (levelKnown)
			return Messages.get(this, "stats_desc", chargesPerCast(), min(), max(), 3 + 2*chargesPerCast(), 30 + 20*chargesPerCast());
		else
			return Messages.get(this, "stats_desc", chargesPerCast(), min(0), max(0), 3 + 2*chargesPerCast(), 30 + 20*chargesPerCast());
	}
